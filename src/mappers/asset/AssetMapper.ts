import { AssetsXML, AssetXML, IAsset, IAssetData, IAssetPalette, PaletteXML } from '../../core';
        ...
        for (const assetXML of xml)
        {
            ...
            if (assetXML.name !== undefined)
            {
                ...
                if (assetXML.source !== undefined)
                {
                    ...
                    if (IMAGE_SOURCES.has(assetXML.source)) asset.source = IMAGE_SOURCES.get(assetXML.source);
                }

                if (assetXML.name !== undefined)
                {
                    if (IMAGE_SOURCES.has(assetXML.name)) asset.source = IMAGE_SOURCES.get(assetXML.name);
                }
