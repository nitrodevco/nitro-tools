...
export const ConvertGenericSwfs = async () => {
    let classNames = ['HabboRoomContent', 'PlaceHolderFurniture', 'PlaceHolderPet', 'PlaceHolderWallItem', 'SelectionArrow', 'TileCursor'];

    if (!classNames) return;

    classNames = (NitroConfiguration.SKIP_CONVERTED_ASSETS ? classNames.filter(x => {
        const filePath = new File(join(NitroConfiguration.OUTPUT_PATH, `./generic/${x}.nitro`));
    ...
    if (!totalItems) {
        console.log('✅ No generic assets to convert! All assets have already been converted.');

        return;
    }

    const spinner = ora(`Starting conversion of ${totalItems} generic assets...`).start();

    let totalConverted = 0;

    const convertItem = async (className: string) => {
        try {
            const buffer = await FetchBuffer({ url: join(NitroConfiguration.OUTPUT_PATH, `./swf/generic/${className}.swf`) });
            const habboAssetSwf = await ExtractSwfFromBuffer(buffer);
            const nitroBundle = await GenerateNitroBundleFromSwf(habboAssetSwf, null);
            const nitroBuffer = await nitroBundle.toArrayBufferAsync();

            await SaveBuffer(Buffer.from(nitroBuffer), `./generic/${className}.nitro`);
    ...
    spinner.succeed(`Conversion complete! ${totalConverted} out of ${totalItems} generic assets converted successfully.`);
};
